...
const QUOTE_CHASE_BODIES: Record<number, Record<NonNullable<Style>, string>> = {
  3: {
...
export function buildQuoteChaseEmail(opts: {
  ...
  const bodyTemplate = QUOTE_CHASE_BODIES[dayMark]?.[s] ?? QUOTE_CHASE_BODIES[21][s];
  const bodyText = bodyTemplate.replace(/\{title\}/g, bookingTitle);

  const subject = `RE: ${bookingTitle} — ${bookingRef}`;
  const body = `${salutation(style, clientName)}\n\n${bodyText}\n\n${close(style)}`;
...
const POST_SHOOT_BODIES: Record<number, Record<NonNullable<Style>, string>> = {
  7: {
...
export function buildPostShootChaseEmail(opts: {
  ...
  const { style, dayMark, bookingRef, clientName } = opts;
  const s = style ?? 'casual';

  const bodyTemplate = POST_SHOOT_BODIES[dayMark]?.[s] ?? POST_SHOOT_BODIES[30][s];
  const bodyText = bodyTemplate.replace(/\{ref\}/g, bookingRef);
...
export function buildBriefClarifyEmail(opts: {
  ...
  if (s === 'formal') {
    intro = `Thank you for forwarding the brief for ${bookingTitle}${bookingRef ? ` (${bookingRef})` : ''}. Before I am able to prepare a quote, I have a few points I would like to clarify:`;
    outro = `Once I have these details I will be able to turn a quote around within a day or two.`;
  } else if (s === 'terse') {
    intro = `${bookingTitle}${bookingRef ? ` (${bookingRef})` : ''} — a few quick questions before I quote:`;
    outro = `Quote to follow once confirmed.`;
  } else {
    // casual — existing voice
    intro = `Thanks for the brief on ${bookingTitle}${bookingRef ? ` (${bookingRef})` : ''}. Before I get a quote together, a few quick clarifications:`;
