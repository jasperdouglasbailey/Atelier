...
- ~~Finance maths fixes + UX cleanup~~ — shipped PR#232 / #233 / #234 / #240 (2026-05-19 / 2026-05-20). **#232** routed GST passthrough to the primary attached artist when a commissionable artist-labour line has `talent_id = null` (fixed a $135 disagreement between FinanceSummary "Agency keeps" and JobPnL "Retained"); **#233** dropped the 8s undo toast on fee-line deletes (annoying on every line edit); **#234** fixed the JobPnL crew-labour double-count (`isReimbursement` was returning true for crew labour, so digi op was counted in both crew + reimb buckets — same bug also tagged every `expense` line as Revenue Drift). **#240** finance-section visual polish — killed the white card backgrounds, toned down the metric tiles (single neutral background + 3px coloured top stripe instead of four ramps), more negative space throughout.
- ~~Quote template surface narrowed to photographer-only~~ — shipped PR#238 / #239 (2026-05-20). #238 dropped the "— TBD" suffix from default line descriptions per Jasper's preference change. #239 retired the videographer / stylist / HMU templates entirely — surface narrowed to one canonical template while the brief flow stabilises. Side-effect: killed the silent super-on-artist_fee bug in the stylist + HMU templates (artist fees should never be super-bearing per doctrine).
- ~~Brief parser + email comms tightening~~ — shipped PR#235 / #237 (2026-05-20). #235 brought in the emoji-bullet crew hold + confirmation email format (super only on the day-fee line; missing fields silently skip), plus travel-rate constants and a bundled fix to the year-inference logic that bumped today's date to next year for the rest of the day after midnight. #237 stopped the LLM critique pass from raising "verify this is talent name, not sender" concerns on canonical enquiry phrasings — operator was dismissing the same false-positive every time.
- ~~Booking deletes at any stage~~ — shipped PR#236 (2026-05-20). Was gated to terminal states (paid / released / cancelled / written_off), so draft and quote-stage bookings had to be transitioned to cancelled first. The type-DELETE confirmation IS the safety layer; the state gate added friction with no real benefit. `deriveOutcome` now classifies all non-won states by the same axis — `quoteWasSent ? lost_post_quote : lost_pre_quote` — giving win-rate analytics a cleaner signal for mid-stage deletes.
