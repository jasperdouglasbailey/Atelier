...
import { addTalentUnavailabilityAction, removeTalentUnavailabilityAction } from '@/app/actions/portal';
import type { TalentUnavailability } from '@/lib/types/database';

type Props = {
  initial: TalentUnavailability[];
};
...
export default function TalentUnavailabilityManager({ initial }: Props) {
  const [rows, setRows] = useState<TalentUnavailability[]>(initial);
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await addTalentUnavailabilityAction(dateFrom, dateTo || dateFrom, reason || null);
      if (!result.ok) { setError(result.error); return; }
      setRows((prev) => [
        ...prev,
        { id: result.id, created_at: new Date().toISOString(), talent_id: '', date_from: dateFrom, date_to: dateTo || dateFrom, reason: reason || null },
      ]);
      setAdding(false);
      setDateFrom('');
      setDateTo('');
  ...
  function handleRemove(id: string) {
    startTransition(async () => {
      const result = await removeTalentUnavailabilityAction(id);
      if (!result.ok) { setError(result.error); return; }
