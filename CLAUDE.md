...
42. **Undo trio: reject-approval + delete-fee-line + delete-schedule-row (PR#119).** ✅ Applies the PR#118 undo pattern to the next three highest-impact one-click actions. New shared `<UndoToast>` component in `src/components/ui/` factors out the visual idiom from PotentialBriefs. (a) **Reject approval**: New `undoRejectApprovalAction` server action sets status back to `pending` (safe because rejection has no external side effects — `applyApprovalDecisionEffects` returns early on decision≠'approved' for every action_type). ApprovalQueue now shows a toast for 8s after reject + a persistent "Restore to pending" button on every rejected row in the Rejected tab. Reason `prompt()` dialog removed — the toast undo is the new safety net. (b) **Delete fee line**: New `restoreFeeLineAction` recreates a deleted line with a new ID; client caches the full FeeLine row pre-delete and replays it on Undo. `confirm()` removed. (c) **Delete schedule row**: client caches the BookingSchedule row, replays via `upsertScheduleAction` on Undo (upsert handles the create case via (booking_id, schedule_date) conflict). `confirm()` removed. Bonus tightening: ApprovalQueue card padding p-4→p-3 + summary spacing mt-1.5→mt-1. Locations empty-state regained inline "Add first location" CTA via new `emptyCta` prop on DenseListTable.

43. **Hotfix: dismissed_by uuid → text (PR#120).** ✅ Migration 0051 declared `atelier_dismissed_brief_candidates.dismissed_by uuid REFERENCES auth.users(id)` but the app uses `getCurrentActor()` which returns email (matching audit_log.user_id text pattern). Inserting Jasper's email into a uuid column crashed with `invalid input syntax for type uuid`. The optimistic-hide reverted on error so dismissed briefs reappeared without refresh — error path working, but write never succeeded. Migration 0052 drops the FK + alters column to text. No code changes needed (TS type already accepts both forms). Migration applied to prod via Supabase MCP.

44. **Dashboard redesign — true snapshot of the platform (PR#121).** ✅ Per Jasper's session-14 redesign notes ("we're missing the entire point of a dashboard"). Six new components in `src/components/dashboard/`: `GreetingHeader` (role-aware Hello/Welcome back, Sydney TZ for time-of-day greeting), `MiniMonthCalendar` (Mon-Sun grid with sand-dot shoot markers, today highlighted, click → /bookings?view=calendar), `FinanceSection` (This week / This month with delta / YTD / Overdue — replaces the previous standalone KPI strip), `ThisWeekStrip` (3 cards: Talent working / Crew on hold (status ≠ confirmed) / Jobs needing crew), `InboxPreview` (last 5 pending approvals), `SettingsSnapshot` (kill switch toggles with useOptimistic + partner accounts link). New data layer `src/lib/data/dashboard.ts`: `getMonthShootMarkers` (handles Postgres daterange exclusive-end), `getThisWeekRoster` (batched), `getJobsNeedingCrew`, `thisWeekRange`, `getBookingsInRange` (client-side intersection — PostgREST overlap op not exposed). GreetingHeader also added to `/portal/talent` and `/portal/crew` so artists/crew get the same greeting on their pages — matching the "applies to agents, artists, crew" requirement.

45. **Dashboard layout v2 — match Jasper's sketch (PR#122).** ✅ Same sections as v1 but reorganised into a tight 3-col / 4-row grid. Row 1: greeting strip (subtle bg). Row 2: Calendar | Upcoming shoots (top) + Crew-on-hold + Jobs-needing-crew (bottom, 50/50 split). Row 3: Pipeline | Inbox. Row 4: Kill switch | Finance (4 metrics). All rows use the same `lg:grid-cols-3` 1+2 split so columns align vertically. Sections dropped per sketch: TalentWorkingThisWeek (duplicative with Upcoming Shoots), TopArtists (moved to /reports), RecentActivity (low signal, in /audit). `ThisWeekStrip` component deleted (no longer used). Conditional rows above row 2: Health banner (failure only), Attention queue (only when items exist).

46. **Dashboard equal heights + inbox briefs preview (PR#123).** ✅ Side-by-side panels in each row now match height. `SectionCard` detects `h-full` in its className and switches to flex-col with `flex-1 min-h-0` content wrapper. All dashboard components (`MiniMonthCalendar`, `InboxPreview`, `SettingsSnapshot`, `FinanceSection`, inline `CrewOnHoldCard`/`JobsNeedingCrewCard`) gained `className` props forwarded to root. Each row wraps cells in `flex` + uses `lg:items-stretch`. Row 2 right column is `flex-col` with Upcoming Shoots auto height + nested Crew+Jobs grid `flex-1`. Plus: InboxPreview now shows potential brief candidates from Gmail — header meta "X pending · Y briefs to triage", body has two labelled subsections (Pending approvals / Potential briefs). New `fetchPotentialBriefsForDashboard()` mirrors `/inbox`'s setup (existingRefs, clientEmails, talentNames, dismissedIds, convertedSourceIds).

47. **Fix: drag-reorder of fee lines persists reliably (PR#124).** ✅ Jasper reported drag-reorder didn't stick — line snapped back after refresh. Root cause: `reorderFeeLinesAction` used `.upsert({ id, sort_order }, { onConflict: 'id' })` with partial columns. PostgREST validates the INSERT side of ON CONFLICT against NOT NULL columns even when every row exists, so partial-column upserts can be silently rejected for NOT NULL columns like `line_type`, `description`, `quantity`. Client ignored the result and `router.refresh()` snapped the order back. Fix: individual `.update({ sort_order }).eq('id', id)` parallel via Promise.all; surface first failure with full Supabase context; client snapshots `feeLines` before optimistic mutation and reverts on failure. Bonus: audit row `fee_line_reorder` on success. Audited GridPlanner — uses local state + localStorage swap, no Supabase write, unaffected.

48. **Fix: upcoming shoots filter + hover card stays on screen (PR#125, refined in PR#126).** ✅ Two bugs. (a) `getUpcomingShoots()` had no date filter — returned bookings sorted by `shoot_dates asc` and the first 10. PR#125 added end-date > today filter. (b) Calendar hover card clipped at month edges + with lots of crew — used `position: absolute` inside `<span>` so inherited parent's overflow:hidden. PR#125 refactored to React portal to document.body + `position: fixed` from `getBoundingClientRect()` + measure-and-flip vertically/horizontally if would overflow viewport. Touch devices skip the popover entirely via `matchMedia('(hover: hover) and (pointer: fine)')`. PR#126 then expanded the state filter (`['quote_confirmed','pre_production','shoot_live']` was too narrow — added all pre-shoot states starting from `brief_received`) and switched to over-fetching 200 unsorted then filtering+sorting in JS so future bookings aren't pushed out by past unadvanced ones. The "common-sense check" was added as a feedback memory.
